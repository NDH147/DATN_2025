...
public class MovieController {
    ...
    public ResponseEntity<ApiPagingResponse<MovieDTO>> getMovieIsEnable(
            ...
            @RequestParam(defaultValue = "100") int size,
            @RequestParam(defaultValue = "releasedDate") String column,
    ...
    public ResponseEntity<?> getMovieBanner(){
        List<MovieDTO> movieDTOS = movieService.getMovieBanner();
        return new ResponseEntity<>(new ApiResponse<>(HttpStatus.OK, movieDTOS, "Danh sách phim banner"), HttpStatus.OK);
    }

    @PutMapping("/movie/update-banner")
    public ResponseEntity<?> updateBanner(@RequestParam Long movieId,
                                          @RequestParam Boolean isBanner){
        String message = movieService.updateIsBanner(movieId, isBanner);
        return new ResponseEntity<>(new ApiResponse<>(HttpStatus.ACCEPTED, null, message), HttpStatus.ACCEPTED);
    }
